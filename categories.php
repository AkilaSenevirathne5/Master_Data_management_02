...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_category'])) {
        ...
        $stmt = $pdo->prepare("SELECT id FROM master_category WHERE code = ?");
        $stmt->execute([$code]);
        if ($stmt->rowCount() > 0) {
        ...
        if (empty($errors)) {
            $stmt = $pdo->prepare("INSERT INTO master_category (code, name, status) VALUES (?, ?, ?)");
    ...
    if (isset($_POST['delete_category'])) {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM master_item WHERE category_id = ?");
        $stmt->execute([$id]);
        $count = $stmt->fetchColumn();
        
        if ($count > 0) {
            $_SESSION['error'] = "Cannot delete category. It is being used by items.";
        } else {
...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM master_category $where");
if (!empty($search)) {
...
$stmt = $pdo->prepare("SELECT * FROM master_category $where ORDER BY created_at DESC LIMIT :limit OFFSET :offset");
if (!empty($search)) {
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex align-items-center mb-3">
                <i class="bi bi-collection text-primary me-3" style="font-size: 2.5rem;"></i>
                <div>
                    <h1 class="h2 mb-0 text-dark fw-bold">Category Management</h1>
                    <p class="text-muted mb-0">Organize your products into logical categories</p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="col-md-5">
                        <div class="mb-3">
                            <label for="name" class="form-label">
                                <i class="bi bi-collection me-1"></i>Name
                            </label>
    ...
    <div class="card slide-in-left" style="animation-delay: 0.2s;">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                ...
                <span class="badge bg-primary ms-2"><?php echo $total; ?> categories</span>
            </div>
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th><i class="bi bi-collection me-1"></i>Name</th>
                        ...
                        <tbody>
                            <?php foreach ($categories as $category): ?>
                                <tr>
                                    <td>
                                        <span class="fw-bold text-primary"><?php echo htmlspecialchars($category['code']); ?></span>
                                    </td>
                                    <td><?php echo htmlspecialchars($category['name']); ?></td>
                                    <td>
                                        <span class="badge bg-<?php echo $category['status'] == 'Active' ? 'success' : 'danger'; ?>">
                                            <i class="bi bi-<?php echo $category['status'] == 'Active' ? 'check-circle' : 'x-circle'; ?> me-1"></i>
